...
import { Loader2, ShieldCheck, ArrowLeft, Fingerprint } from "lucide-react";
...
export default function Verify2FAPage() {
  ...
  useEffect(() => {
    const creds = api.auth.getPending2FA() as PendingCredentials | null;
    if (!creds) {
  ...
  async function handleVerify(e?: React.FormEvent) {
    ...
    try {
      ...
      const routes: Partial<Record<UserRole, string>> = {
        ...
        [UserRole.ADMIN]: "/admin", // Assuming admin route exists or falls back
      };

      const targetRoute =
        returnTo || routes[user.role as UserRole] || "/onboarding/role";
  ...
  return (
    <div className="min-h-screen bg-[#09090b] text-zinc-100 flex flex-col items-center justify-center p-6 selection:bg-emerald-500/30">
      ...
      <div className="w-full max-w-[440px] relative z-10">
        ...
        {/* Main Card */}
        <div className="bg-zinc-900/40 border border-zinc-800/50 backdrop-blur-xl p-10 rounded-4xl shadow-2xl">
          <form onSubmit={handleVerify} className="space-y-8">
            ...
            {error && (
              <div className="bg-red-500/5 border border-red-500/20 py-3 px-4 rounded-xl">
                <p className="text-xs text-center font-medium text-red-400">
                  {error}
                </p>
              </div>
            )}
