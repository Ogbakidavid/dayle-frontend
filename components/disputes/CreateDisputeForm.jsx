...
export function CreateDisputeForm({ role, initialVaultId }) {
    ...
    const selectedMilestone = useMemo(() => {
        return availableMilestones.find((m) => m.id === selectedMilestoneId) || null;
    }, [availableMilestones, selectedMilestoneId]);

    const selectedRequirement = useMemo(() => {
        return (selectedMilestone?.requirements || []).find((r) => r.reqId === selectedRequirementId) || null;
    ...
    const step1Complete = Boolean(selectedVaultId && selectedMilestoneId && selectedMilestone);
    ...
    const validateAndAddFiles = useCallback((incoming) => {
        ...
        for (const f of incoming) {
            ...
            if (f.size > MAX_FILE_BYTES) {
                setFileError(`File too large. Max allowed is ${MAX_FILE_MB}MB per file.`);
                continue;
            }
            next.push(f);
        }
        ...
        if (next.length) {
            setFiles((prev) => {
                const seen = new Set(prev.map((x) => `${x.name}_${x.size}_${x.lastModified}`));
    ...
    const handleSubmit = async (e) => {
        ...
        if (!selectedMilestoneId || !selectedMilestone) return setFormError('Select a milestone.');
        if (!eligibility?.eligible) return setFormError('This milestone is not eligible for dispute.');
        ...
        <div className="mx-auto max-w-3xl space-y-6 px-3 md:px-4 pb-10 pt-6">
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                ...
                <div className="flex items-center gap-2 text-xs text-white/40">
                    <span className={cx('h-2 w-2 rounded-full', step1Complete ? 'bg-emerald-400' : 'bg-white/20')} />
                    <span>Step 1</span>
                    <span className="mx-1">•</span>
                    <span className={cx('h-2 w-2 rounded-full', step2Ready ? 'bg-emerald-400' : 'bg-white/20')} />
                    <span>Step 2</span>
                </div>
            </div>
            ...
            <header className="space-y-2">
                <h1 className="text-2xl md:text-3xl font-semibold tracking-tight text-white">Open a dispute</h1>
                <p className="text-sm leading-relaxed text-white/60">
                    Disputes are scoped to a specific milestone and a reason code. Choose carefully. Weak or vague disputes get rejected.
                </p>
            </header>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                                ...
                                >
                                    <SelectTrigger className="h-11 w-full border-white/10 bg-black/40 text-white hover:border-white/20 focus:ring-2 focus:ring-amber-500/30">
                                        <SelectValue placeholder="Select a vault...">
                                            {selectedVault ? (
                                                <span className="block w-full truncate">{selectedVault.title}</span>
                                            ) : null}
                                        </SelectValue>
                                    </SelectTrigger>
                    ...
                    "
                                    >
                                        {availableVaults.map((vault) => (
                                            <SelectItem key={vault.id} value={vault.id} className="focus:bg-white/10 focus:text-white">
                                                <div className="flex w-full items-center justify-between gap-3">
                                                    <span className="truncate">{vault.title}</span>
                                                    <span className="shrink-0 text-[11px] text-white/40">{vault.id}</span>
                                                </div>
                                            </SelectItem>
                                        ))}
                                    </SelectContent>
                                </Select>
                                <p className="text-xs text-white/40">This determines the dispute jurisdiction and parties.</p>
                            </div>

                            {/* Milestone */}
                            <div className="space-y-2">
                                <Label className="text-sm font-medium text-white">Milestone</Label>
                                <Select
                                ...
                                >
                                    <SelectTrigger className="h-11 w-full border-white/10 bg-black/40 text-white hover:border-white/20 disabled:opacity-50 focus:ring-2 focus:ring-amber-500/30">
                                        <SelectValue placeholder={selectedVaultId ? 'Select a milestone...' : 'Select a vault first'}>
                                            {selectedMilestone ? (
                                                <span className="block w-full truncate">{selectedMilestone.title}</span>
                                            ) : null}
                                        </SelectValue>
                                    </SelectTrigger>
                    ...
                    "
                                    >
                                        {availableMilestones.length === 0 ? (
                                            <div className="p-3 text-sm text-white/50">No milestones found.</div>
                                        ) : (
                                ...
                                <p className="text-xs text-white/40">Disputes are tied to one milestone only.</p>
                            </div>
                        </div>
                        ...
                        {selectedMilestone ? (
                            <div className="rounded-xl border border-white/10 bg-white/[0.03] p-4">
                                <div className="flex items-start justify-between gap-4">
                                    <div className="min-w-0">
                                        <div className="text-sm font-semibold text-white">{selectedMilestone.title}</div>
                                        <div className="mt-1 text-xs text-white/50">
                                            Status: <span className="text-white/70">{selectedMilestone.status}</span> • Type:{' '}
                                            <span className="text-white/70">{selectedMilestone.type}</span>
                                        </div>
                                    </div>
                ...
                {step1Complete ? (
                    <Card className="border-white/10 bg-[#0B0B0C]">
                        <CardHeader className="space-y-3">
                            <StepHeader step="2" title="Details & evidence" subtitle="Pick a reason code and attach supporting proof." />
                        </CardHeader>

                        <CardContent className="space-y-6 mt-4">
                            <StatusBanner
                                eligible={Boolean(eligibility?.eligible)}
                                title={eligibility?.eligible ? 'Eligible for dispute' : 'Not eligible'}
                                description={eligibility?.reason || 'Eligibility could not be determined.'}
                            />

                            {/* Requirement (conditional) */}
                            {eligibility?.eligible &&
                                (eligibility?.requiresRequirementSelection || (selectedMilestone?.requirements || []).length > 0) ? (
                                <div className="space-y-2">
                                    <Label className="text-sm font-medium text-white">
                                        Related requirement{' '}
                                        <span className="text-xs font-normal text-white/40">(optional unless required)</span>
                                    </Label>
                                    ...
                                    <Select value={selectedRequirementId} onValueChange={setSelectedRequirementId}>
                                        <SelectTrigger className="h-11 w-full border-white/10 bg-black/40 text-white hover:border-white/20 focus:ring-2 focus:ring-amber-500/30">
                                            <SelectValue placeholder="Select requirement...">
                                                {selectedRequirement ? (
                                                    <span className="block w-full truncate">{selectedRequirement.reqId}</span>
                                                ) : null}
                                            </SelectValue>
                                        </SelectTrigger>
                      ...
                      "
                                        >
                                            {(selectedMilestone?.requirements || []).map((req) => (
                                                <SelectItem key={req.reqId} value={req.reqId} className="focus:bg-white/10 focus:text-white">
                                                    <div className="flex flex-col items-start py-1">
                                                        <span className="text-sm font-medium">{req.reqId}</span>
                                                        <span className="text-xs text-white/40">{req.label}</span>
                                                    </div>
                                                </SelectItem>
                                            ))}
                                        </SelectContent>
                                    ...
                                    <p className="text-xs text-white/40">
                                        If your dispute is about a specific deliverable requirement, attach it here.
                                    </p>
                                </div>
                            ) : null}
                            ...
                            {eligibility?.eligible ? (
                                <div className="space-y-3">
                                    <div className="flex items-end justify-between gap-3">
                                        ...
                                        <div className="text-xs text-white/40">Choose one. Make it defensible.</div>
                                    </div>
                                                ...
                                                >
                                                    ...
                                                    <div className="min-w-0 flex-1">
                                                        <div className="flex items-center justify-between gap-3">
                                                            <div className="text-sm font-semibold text-white">{code.label}</div>
                                                            <div
                                                                className={cx(
                                                        ...
                                                        <div className="mt-1 text-sm text-white/60">{code.description}</div>
                                                    </div>
                                                </label>
                                            );
                                        })}
                            ...
                            {eligibility?.eligible && selectedReasonCode ? (
                                <div className="space-y-6 border-t border-white/10 pt-6">
                                    <div className="space-y-2">
                                        <div className="flex items-end justify-between gap-3">
                                            <Label className="text-sm font-medium text-white">
                                                Summary <span className="text-xs font-normal text-white/40">(optional)</span>
                                            </Label>
                                            <div className="text-xs text-white/40">{summary.length}/600</div>
                                        </div>
                                        ...
                                        />
                                        <p className="text-xs text-white/40">
                                            Good disputes read like a report: facts, dates, evidence. No drama.
                                        </p>
                                    </div>
                                    ...
                                    <div className="space-y-3">
                                        ...
                                        >
                                            <div className="flex items-start gap-4">
                                                ...
                                                <div className="min-w-0">
                                                    <div className="text-sm font-semibold text-white">Drag and drop files here</div>
                ...
                <div className="flex flex-col gap-3 rounded-xl border border-white/10 bg-white/[0.02] p-4 sm:flex-row sm:items-center sm:justify-between">
                    <div className="flex items-start gap-2 text-xs text-white/45">
                        ...
                        <p>
                            Submitting a dispute means you're asserting the information is accurate. Poor evidence and vague claims will get denied.
                        </p>
                    </div>
