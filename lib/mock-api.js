...
// Initialize mockLedger with seed data
let mockLedger = {
  available: 12450.00,
  pending: 2500.00,
  transactions: [
    {
      ...
      description: "Withdrawal to Bank (...4421)"
    },
    {
      id: "RLS-4421-Y",
      ...
      amount: 4500.00,
      status: TransactionStatus.CONFIRMED,
      description: "Vault Milestone Release: Smart Contract Audit"
    }
  ]
};
...
export const api = {
  ...
  ledger: {
    getBalance: async () => {
      await sleep(DELAY_MS);
      // Return real mockLedger state
      const available = mockLedger.available || 0;
      const pending = mockLedger.pending || 0;
      return { 
        available, 
        pending, 
    ...
    withdraw: async (amount, opts = {}) => {
      ...
      if (!idempotencyKey) throw new Error("Idempotency key is required for money-moving actions");

      const withdrawAmount = parseFloat(amount);
      const currentAvailable = mockLedger.available || 0;
      ...
      // Mutation
      mockLedger.available = currentAvailable - withdrawAmount;
      mockLedger.pending = (mockLedger.pending || 0) + withdrawAmount;

      const tx = {
        id: `WDL-${Math.random().toString(36).substr(2, 9).toUpperCase()}`,
        type: 'WITHDRAW',
      ...
      mockLedger.transactions = mockLedger.transactions || [];
      mockLedger.transactions.unshift(tx);

      // Simulate confirmation after 5 seconds: available-pending -> actually gone
      setTimeout(() => {
        const txIndex = mockLedger.transactions.findIndex(t => t.id === tx.id);
        if (txIndex !== -1 && mockLedger.transactions[txIndex].status === TransactionStatus.PENDING) {
          mockLedger.transactions[txIndex].status = TransactionStatus.CONFIRMED;
          mockLedger.transactions[txIndex].completedAt = new Date().toISOString();
          mockLedger.pending = Math.max(0, mockLedger.pending - withdrawAmount);
        }
      }, 5000);
    ...
    getTransactions: async () => {
      await sleep(800);
      // Return real mockLedger transactions
      return mockLedger.transactions || [];
    },
  },
  vaults: {
    list: async () => {
    ...
    fund: async (vaultId, data) => {
      ...
      // Else go to FUNDED
      vault.status = vault.freelancerId ? VaultStatus.FUNDED : VaultStatus.FUNDED;
      if (!vault.freelancerId) {
          // In real system, FUNDED implies awaiting assignment if freelancerId is null
          // but for now let's just use FUNDED
      }
    ...
    releaseMilestone: async (vaultId, milestoneId, opts = {}) => {
      ...
      milestone.releaseStatus = ReleaseStatus.SUBMITTED; // Money action started
      milestone.releasedAt = new Date().toISOString();

      // Simulate confirmation
      setTimeout(() => {
          milestone.releaseStatus = ReleaseStatus.CONFIRMED;
    ...
    refund: async (vaultId, milestoneId, opts = {}) => {
      ...
      const vault = mockVaults.find((v) => v.id === vaultId);
      if (!vault) throw new Error('Vault not found');
      if (vault.clientId !== mockUser.id && mockUser.role !== UserRole.ADMIN) throw new Error('Unauthorized');

      const milestone = (vault.milestones || []).find((m) => m.id === milestoneId);
      if (!milestone) throw new Error('Milestone not found');

      // STATE TRANSITION GUARD: Typically refund happens after REJECTED
      if (milestone.status !== MilestoneStatus.REJECTED && milestone.status !== MilestoneStatus.DISPUTED) {
         // Allow refund if status is REJECTED or DISPUTED (admin outcome)
         // For client self-serve, REJECTED is the main trigger
         throw {
            code: "INVALID_STATE_TRANSITION",
            message: `Cannot refund milestone from status "${milestone.status}". Must be "${MilestoneStatus.REJECTED}" or "${MilestoneStatus.DISPUTED}".`,
            currentStatus: milestone.status,
            allowedStatuses: [MilestoneStatus.REJECTED, MilestoneStatus.DISPUTED],
      ...
      // Create a ledger-like REFUND object
      const ledgerEntry = {
        id: `LG_REF_${Date.now()}`,
        createdAt: new Date().toISOString(),
        vaultId,
        ...
        status: LedgerEntryStatus.PENDING,
        amount: -(Number(milestone.amount) || 0), // Negative for display or positive logic depending on ledger model? usually Refund is money back to Client, so +ClientBalance
        // Architecture says: Refund Confirmed: Post LedgerEntry(REFUND, NEGATIVE amount) ??
        // Actually, ARCHITECTURE says: "Refund Confirmed: Post LedgerEntry(REFUND, NEGATIVE amount)"
        // But usually Refund means Credit to Client.
        // Let's stick to ARCHITECTURE text: "Refund Confirmed: Post LedgerEntry(REFUND, NEGATIVE amount)" implies it reverses the "LOCK" or "RELEASE"?
        // Wait, "RELEASE" reduces client balance? No, client balance is reduced on DEPOSIT.
        // Escrow has the money.
        // Refund moves money Escrow -> Client Balance.
        // So this ledger entry should reflect an INCREASE in Client Balance?
        // Let's assume the LedgerContext sums up entries.
        // Release = +Freelancer Balance? No, "Release moves money".
        // Let's assume for this mock, REFUND means client gets money back.
        // Implementation detail: we'll follow the pattern of Release but localized to Refund.
        currency: 'USD',
        description: `Refund for ${milestone.title}`,
      };
      
      // Update mock ledger balance for client
      // (This is a simplification, real system would be more complex)
      mockLedger.available = (mockLedger.available || 0) + (Number(milestone.amount) || 0);

      // Milestone status logic: Does it change?
      // "Refund is a separate explicit money action".
      // Status might stay REJECTED, but maybe we track "refundStatus"?
      // Let's use `releaseStatus` or similar `refundStatus` if we had it.
      // For now, we'll assume the milestone is practically "Settled" as refunded.
      // But we won't change the main status from REJECTED so history is preserved.
      // We'll mark it as refunded in a new property for UI helpers.
      milestone.refundStatus = ReleaseStatus.SUBMITTED;
      milestone.refundedAt = new Date().toISOString();

      setTimeout(() => {
          milestone.refundStatus = ReleaseStatus.CONFIRMED;
          ledgerEntry.status = LedgerEntryStatus.CONFIRMED;
      }, 2000);
