...
export function KYCAlert() {
    ...
    if (loading || !user || user.kycStatus === 'approved') return null;
                    ...
                    <div className="space-y-1">
                        <h3 className="text-lg font-black uppercase tracking-tight text-white flex items-center gap-2">
                            ...
                            <span className="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold bg-amber-500/20 text-amber-500 uppercase tracking-wide border border-amber-500/20">
                                {user.kycStatus === 'rejected' ? 'Verification Failed' : 'Pending Verification'}
                            </span>
                        </h3>
                        <p className="text-amber-200/60 text-sm font-medium leading-relaxed max-w-2xl">
                            To unlock full platform capabilities including withdrawals and vault creation, please complete your identity verification. This helps us ensure a secure environment for all users.
                        </p>
                    </div>
                </div>
