...
import {
    ...
    Download, RefreshCw, FileText, CheckCircle,
    XCircle, Clock
} from 'lucide-react';
...
const mockTransactions = [
    { id: 'TX-98234', type: 'deposit', counterparty: 'External Wallet', status: 'completed', amount: 5000, date: 'Oct 24, 2025', method: 'USDC' },
    { id: 'TX-98233', type: 'payment', counterparty: 'alex@devstudio.com', status: 'completed', amount: 2500, date: 'Oct 23, 2025', method: 'Escrow Release' },
    { id: 'TX-98232', type: 'payment', counterparty: 'sarah@design.co', status: 'pending', amount: 1500, date: 'Oct 22, 2025', method: 'Escrow Release' },
            ...
            <div className="max-w-7xl mx-auto px-6 space-y-10">
                ...
                <header className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                    <div className="space-y-1">
                        ...
                        <p className="text-sm text-slate-500 font-medium">Financial activity and history</p>
                    </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div className="bg-[#0D0D0E] border border-white/5 p-6 rounded-2xl">
                        ...
                        <h2 className="text-3xl font-bold text-emerald-500 mt-1">$31,000</h2>
                    </div>
                </div>
                ...
                <div className="relative group">
                    ...
                    <input
                        ...
                        className="w-full pl-12 pr-4 py-4 bg-[#0D0D0E] border border-white/5 rounded-sm focus:outline-none focus:border-emerald-500/50 text-white placeholder-slate-600 transition-all shadow-inner"
                    />
                </div>
                            ...
                            <tbody className="divide-y divide-white/5">
                                {paginatedTransactions.map((tx) => (
                                    <tr key={tx.id} className="group hover:bg-white/[0.02] transition-colors">
                                        ...
                                        <td className="px-6 py-5">
                                            <span className="text-sm text-slate-400 capitalize">{tx.type}</span>
                                        </td>
                                        <td className="px-6 py-5">
                                            <span className="text-sm text-slate-400 font-medium">{tx.date}</span>
                                        </td>
                                        <td className="px-6 py-5">
                                            <div className={cn(
                                                "inline-flex items-center gap-1.5 px-2.5 py-1 rounded-sm border text-[10px] font-bold uppercase tracking-wider",
                                                tx.status === 'completed' ? "bg-emerald-500/5 border-emerald-500/20 text-emerald-500" :
                                                    tx.status === 'processing' || tx.status === 'pending' ? "bg-amber-500/5 border-amber-500/20 text-amber-500" :
                                                        "bg-red-500/5 border-red-500/20 text-red-500"
                                            )}>
                                                <div className={cn("w-1 h-1 rounded-full",
                                                    tx.status === 'completed' ? "bg-emerald-500" :
                                                        tx.status === 'processing' || tx.status === 'pending' ? "bg-amber-500" : "bg-red-500"
                                                )} />
                                                {tx.status}
                                            </div>
                                        </td>
                                        <td className="px-6 py-5 text-right">
                                            ...
                                            <p className="text-[10px] text-slate-600 font-bold uppercase">{tx.method}</p>
                                        </td>
                                    </tr>
                                ))}
                            </tbody>
