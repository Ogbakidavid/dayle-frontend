            ...
            <div className="flex flex-col lg:flex-row min-h-screen">
                ...
                <section className="w-full lg:w-[25%] bg-[#080808] p-10 border-r border-white/5 flex flex-col justify-between">
                    ...
                    <div className="p-6 bg-emerald-500/5 border border-emerald-500/10 rounded-2xl">
                        ...
                        <p className="text-sm text-white leading-relaxed font-bold">Funds are held in a secure multi-sig vault until milestone approval.</p>
                    </div>
                </section>
                    ...
                    <div className="max-w-md w-full">
                        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-8">
                            <div className="text-center space-y-2">
                                ...
                                <p className="text-white font-bold uppercase tracking-wide text-sm">Select how you want to fund this project</p>
                            </div>
...
function MethodBtn({ icon, title, desc, onClick }) {
    return (
        <button onClick={onClick} className="w-full p-6 bg-white/[0.03] border border-white/5 rounded-3xl flex items-center gap-6 group hover:bg-emerald-500/5 hover:border-emerald-500/30 transition-all text-left">
            <div className="w-14 h-14 bg-white/5 rounded-2xl flex items-center justify-center text-white group-hover:bg-emerald-500 group-hover:text-black transition-all">{icon}</div>
            <div className="flex-1">
                <p className="text-white font-bold text-lg">{title}</p>
                <p className="text-sm text-white font-bold">{desc}</p>
            </div>
            <ChevronRight className="w-5 h-5 text-white group-hover:text-white group-hover:translate-x-1 transition-all" />
        </button>
    );
}