...
export default function VaultsPage() {
    ...
    const filteredVaults = (vaults || []).filter((vault: any) => {
        const counterparty =
            vault.freelancerName || vault.freelancerEmail || vault.freelancer?.email || "";
        return (
    ...
    const totalValue = (vaults || []).reduce((acc: number, v: any) => acc + (v.totalAmount || v.amount || 0), 0);
    const activeCount = (vaults || []).filter((v: any) => v.status === "active").length;
    const completionRate = (vaults || []).length > 0
        ? Math.round(((vaults || []).filter((v: any) => v.status === "completed").length / (vaults || []).length) * 100)
        : 0;
    ...
    return (
        <motion.div
            ...
            initial="hidden"
            animate="visible"
            className="min-h-screen text-gray-400 font-sans selection:bg-emerald-500/30 font-['Poppins',sans-serif]"
        >
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full text-left border-collapse min-w-[640px]">
                            ...
                            <tbody className="divide-y divide-white/5">
                                {loading ? (
                                    <tr>
                                        <td colSpan={5} className="px-6 py-10 text-center text-gray-500 font-bold uppercase tracking-wide">
                                            Loading vaults...
                                        </td>
                                    </tr>
                                                ...
                                                <div className="flex items-center gap-3 md:gap-4">
                                                    ...
                                                    <div className="min-w-0">
                                                        ...
                                                        <p className="lg:hidden text-[10px] text-gray-400 font-bold uppercase tracking-wide mt-1 truncate">
                                                            {vault.freelancerName || vault.freelancerEmail || vault.freelancer?.email || "Unassigned"}
                                                        </p>
                                                    </div>
                                                </div>
                                            </td>
