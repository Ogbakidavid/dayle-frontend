...
import type { Milestone } from '@/lib/store/vault-context';
...
export function DisputeInitiationPanel({ milestone }: DisputeInitiationPanelProps) {
    const [selectedRequirement, setSelectedRequirement] = useState('');
    const [selectedReasons, setSelectedReasons] = useState<string[]>([]);

    const policy: DisputePolicy = milestone?.disputePolicy || {
        eligibleStatuses: ['failed', 'rejected'],
        requiresRequirementId: true
    };
    
    const eligibility = useMemo(
        () => getDisputeEligibility(milestone as any, selectedRequirement || null),
    ...
    return (
        <Card className="bg-[#111111] border-white/10 shadow-xl overflow-hidden">
            ...
            <CardContent className="space-y-6 pt-6">
                ...
                {milestone && policy.requiresRequirementId && (
                    <div className="space-y-2">
                        <p className="text-[10px] font-bold uppercase tracking-widest text-white/40">Requirement scope</p>
                        <select
                        ...
                        >
                            ...
                            {(milestone as any).requirements?.map((req: any) => (
                ...
                <div className="space-y-3">
                    <p className="text-[10px] font-bold uppercase tracking-widest text-white/40">Reason codes</p>
                            ...
                            return (
                                <label 
                                    ...
                                    className={`flex items-start gap-4 border transition-all cursor-pointer rounded-xl p-4 text-sm ${
                                        ...
                                        : 'border-white/5 bg-black/40 text-white/60 hover:border-white/20 hover:bg-white/[0.04]'
                                    }`}
                                >
                                    <input
                                        type="checkbox"
                                    ...
                                    <div className="space-y-1">
                                        <p className="text-sm font-semibold text-white/90">{reason.label}</p>
                ...
                <div className={`flex items-start gap-3 rounded-xl border p-4 transition-all ${
                    ...
                    <EligibilityIcon className={`w-5 h-5 mt-0.5 shrink-0 ${eligibilityTone}`} />
                    <div className="min-w-0">
                        <p className="text-sm font-semibold">{eligibility.eligible ? 'Ready to Open' : 'Action Required'}</p>
                        <p className="text-xs text-white/70">{eligibility.reason || 'Select scope above'}</p>
                    </div>
                </div>
