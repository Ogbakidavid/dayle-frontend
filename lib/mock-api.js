...
export const VaultStatus = {
  ...
  INVITED: "INVITED", // Invite sent, no funds
  FUNDED_UNASSIGNED: "FUNDED_UNASSIGNED", // Funded, waiting for freelancer
  FUNDED_ASSIGNED: "FUNDED_ASSIGNED", // Funded, freelancer accepted
  AWAITING_FUNDING: "AWAITING_FUNDING", // Freelancer accepted, waiting for funds
  ACTIVE: "active", // Funded, work in progress (Legacy/Simplified)
...
export const api = {
  auth: {
    ...
    signup: async (email, password) => {
      await sleep(DELAY_MS);
      // For demo purposes, we default new users to Freelancer role
      // if they signed up from an invite link, otherwise they go to onboarding.
      const nameFromEmail = email.split("@")[0];
      mockUser = {
        ...mockUser,
        email,
        name: nameFromEmail.charAt(0).toUpperCase() + nameFromEmail.slice(1),
        id: `u_${Math.random().toString(36).substr(2, 9)}`,
        role: UserRole.FREELANCER, // Set default role for invited users
        kycStatus: KycStatus.NONE
      };
      if (typeof window !== "undefined") {
        localStorage.setItem(SESSION_KEY, JSON.stringify(mockUser));
    ...
    respond: async (token, { decision, reasonCode }) => {
      ...
      if (decision === "ACCEPT") {
        ...
        if (vault.status === VaultStatus.FUNDED_UNASSIGNED) {
          ...
          vault.status = VaultStatus.AWAITING_FUNDING;
        }
