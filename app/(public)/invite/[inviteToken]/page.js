...
export default function InvitePage() {
    ...
    const [data, setData] = useState(null); // { invite, vault }
    ...
    useEffect(() => {
        async function loadData() {
            try {
                ...
                // 2. Get Invite Data
                if (inviteToken) {
                    const result = await api.invites.getByToken(inviteToken);
                    setData(result);
                }
            } catch (err) {
    ...
    // 'Invite expired' is thrown by getByToken, so it comes here as error
    if (error) {
    ...
    if (data?.invite?.status !== "PENDING") {
        ...
        return (
            <div className="min-h-screen bg-[#050505] flex flex-col items-center justify-center p-6">
                <div className="max-w-md w-full text-center space-y-6">
                    <div className={`w-20 h-20 rounded-full flex items-center justify-center mx-auto border ${status === 'ACCEPTED' ? 'bg-emerald-500/10 border-emerald-500/20' : 'bg-red-500/10 border-red-500/20'}`}>
                        {status === 'ACCEPTED' ? <CheckCircle2 className="w-10 h-10 text-emerald-500" /> : <XCircle className="w-10 h-10 text-red-500" />}
                    </div>
                    <h1 className="text-3xl font-black text-white uppercase tracking-tighter">
                    ...
                    {status === 'ACCEPTED' && (
                        <div className="pt-4">
                            <Button
                                onClick={() => router.push(`/freelancer/vault/${data.vault.id}`)}
    ...
    const isLoggedIn = currentUser?.role !== UserRole.NONE;

    return (
        <div className="min-h-screen bg-[#050505] font-['Poppins',_sans-serif] selection:bg-emerald-500/30">
            {/* Simple Header */}
                    ...
                    {isLoggedIn && (
                        <div className="flex items-center gap-4">
                            <div className="flex items-center gap-3">
                                ...
                                <span className="text-sm font-bold text-white/50">{currentUser.email}</span>
                            </div>
                            <button
                                onClick={async () => {
            ...
            <main className="container mx-auto px-4 py-12 md:py-20 flex flex-col items-center">
                ...
                <div className="w-full max-w-4xl grid md:grid-cols-2 gap-6 items-start">
                    ...
                    <div className="bg-[#0A0A0A] border border-white/5 rounded-2xl p-6 md:p-8 space-y-8 relative overflow-hidden group">
                        <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
                            <Shield className="w-48 h-48 text-white" />
                        </div>

                        <div className="relative z-10">
                            <h3 className="text-xs font-black uppercase tracking-widest text-white/40 mb-6">Safe Project Summary</h3>

                            <div className="grid grid-cols-2 gap-6">
                                <div>
                                    <p className="text-white/40 text-xs font-bold uppercase tracking-wide mb-1">Total Value</p>
                                ...
                                <div>
                                    <p className="text-white/40 text-xs font-bold uppercase tracking-wide mb-1">Milestones</p>
                                    <div className="text-3xl font-black text-white tracking-tight flex items-baseline gap-2">
                                        {vault.milestoneCount}
                                        <span className="text-sm font-bold text-white/30 uppercase">Phases</span>
                                    </div>
                                </div>
                            </div>
                    ...
                    {/* RIGHT: Action Card */}
                    <div className="bg-[#0A0A0A] border border-white/5 rounded-2xl p-6 md:p-8 flex flex-col justify-center min-h-[300px]">
                        ...
                        {!isLoggedIn ? (
                            <div className="text-center space-y-6">
                                ...
                                <div>
                                    <h3 className="text-xl font-bold text-white uppercase tracking-tight italic">Join to Respond</h3>
                                    <p className="text-white/50 text-sm mt-2 font-medium">Create an account or sign in to accept this project invitation.</p>
                                </div>
                            ...
                            <div className="text-center space-y-6">
                                ...
                                <div>
                                    <h3 className="text-xl font-bold text-white uppercase tracking-tight">Freelancer Access Only</h3>
                                    <p className="text-white/50 text-sm mt-2">You are logged in as a Client. This invitation is intended for a Freelancer account.</p>
                                </div>
                                <div className="grid gap-3">
                                    <Button onClick={() => router.push('/client')} variant="outline" className="w-full h-12 border-white/10 hover:bg-white/5 text-white font-bold uppercase tracking-wide rounded-xl transition-all">
                                        Return to Dashboard
                                    </Button>
                                    <Button
                                        onClick={async () => {
                            ...
                            <div className="text-center space-y-6">
                                ...
                                <div>
                                    <h3 className="text-xl font-bold text-white uppercase tracking-tight italic text-red-400">Identity Mismatch</h3>
                                    <p className="text-white/50 text-sm mt-2 font-medium leading-relaxed">
                                        This invitation was sent to <span className="text-white font-bold">{invite.email}</span>,
                                        but you are logged in as <span className="text-white font-bold">{currentUser.email}</span>.
                                    </p>
                                </div>
                                <Button
                                    onClick={async () => {
                            ...
                            <div className="space-y-6">
                                <div>
                                    <h3 className="text-xl font-bold text-white uppercase tracking-tight">Ready to collaborate?</h3>
                                    <p className="text-white/50 text-sm mt-2">
                                        Accepting creates a binding workspace. You can review full requirements before confirming any milestones.
                                    </p>
                                </div>
                                ...
                                <div className="grid gap-3">
                                    ...
                                    >
                                        {processing ? <Loader2 className="animate-spin" /> : "Accept Invitation"}
                                    </Button>
                ...
                {showDeclineModal && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
                        <motion.div
                            ...
                            className="w-full max-w-sm bg-[#111] border border-white/10 rounded-2xl p-6 space-y-6"
                        >
                            <div>
                                <h3 className="text-lg font-bold text-white uppercase tracking-tight">Decline Invitation</h3>
                                <p className="text-white/50 text-sm mt-1">Please select a reason for declining.</p>
                            </div>
                            ...
                            <div className="space-y-2">
                                {DECLINE_REASONS.map((reason) => (
                                    <button
                                        ...
                                        className={`w-full text-left p-3 rounded-lg border text-sm font-medium transition-all ${declineReason === reason.id
                            ...
                            <div className="flex gap-3 pt-2">
                                ...
                                >
                                    {processing ? <Loader2 className="w-4 h-4 animate-spin" /> : "Confirm Decline"}
                                </Button>
                            </div>
                        </motion.div>
                    </div>
