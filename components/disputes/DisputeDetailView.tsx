...
export function DisputeDetailView({ disputeId, role }: DisputeDetailViewProps) {
  ...
  if (!dispute) return (
    <div className="py-20 text-center space-y-4">
      <AlertCircle className="w-12 h-12 text-slate-200 mx-auto" strokeWidth={1} />
      <p className="text-slate-400 font-bold">Dispute record not found.</p>
      <Link href={`/${role}/disputes`}>
        <Button variant="ghost" className="text-amber-600 font-bold">Return to Resolution Center</Button>
      </Link>
    </div>
  );
  ...
  const currentStepIndex = statusSteps.findIndex(s => 
    s.key === dispute.status || (dispute.status === "investigating" && s.key === "UNDER_REVIEW")
  );
  ...
  return (
    <div className="space-y-8 max-w-6xl mx-auto pb-20">
            ...
            >
              ...
              {submitted ? (
                <div className="text-center py-10 space-y-4">
                   ...
                   <h3 className="text-xl font-black text-slate-900">Submitted</h3>
                   <p className="text-sm text-slate-400 font-bold">Your evidence has been added to the case ledger.</p>
                </div>
              ) : (
                <div className="relative z-10 space-y-8">
                  <header>
                    <h2 className="text-2xl font-black text-slate-900 tracking-tight">Add Evidence</h2>
                    <p className="text-sm text-slate-400 font-medium">Support your case with files, links, or notes.</p>
                  </header>
                    ...
                    {(["link", "note", "file"] as EvidenceType[]).map((t) => (
                      <button
                        ...
                        className={cn(
                          ...
                          evidenceType === t ? "bg-white text-slate-900 shadow-sm ring-1 ring-slate-100" : "text-slate-400 hover:text-slate-600"
                        )}
                      >
                        {t}
                      </button>
                    ...
                    {(evidenceType === "link" || evidenceType === "file") && (
                      <div className="space-y-2">
                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] px-1">
                          {evidenceType === "link" ? "Reference URL" : "File Identifier"}
                        </label>
                        <input
                          placeholder={evidenceType === "link" ? "https://..." : "e.g. screenshot.png"}
                          value={evidenceValue}
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <Link href={`/${role}/disputes`} className="inline-flex items-center gap-2 group text-slate-400 hover:text-slate-900 transition-colors font-black uppercase tracking-tighter text-xs">
        ...
        <div className="flex items-center gap-4">
           {dispute.status === "OPEN" && (
             <Button onClick={() => setShowModal(true)} className="bg-emerald-600 hover:bg-emerald-700 text-white font-black h-12 px-6 rounded-2xl shadow-lg shadow-emerald-600/20">
               + Add Evidence
             </Button>
           )}
        </div>
         ...
         <div className="relative z-10 space-y-6">
            <div className="space-y-1">
               ...
               <h1 className="text-3xl md:text-5xl font-black tracking-tight leading-none">
                 Case <span className="text-white/30 text-2xl md:text-4xl">#</span>{dispute.id.slice(0, 12).toUpperCase()}
               </h1>
            </div>
            ...
            <div className="pt-8 grid grid-cols-4 gap-2 relative">
               ...
               <div className="absolute top-[18px] left-[12.5%] transition-all duration-1000 bg-emerald-400 h-px" style={{ width: `${Math.max(0, currentStepIndex) * 25}%` }} />
                 ...
                 return (
                   <div key={i} className="flex flex-col items-center gap-4 relative z-10">
                      <div className={cn(
                        ...
                        isCurrent ? "bg-emerald-400 border-emerald-400 scale-110 shadow-emerald-400/20" : 
                        isActive ? "bg-white border-white shadow-white/10" : "bg-slate-800 border-white/5"
                      )}>
                        {isActive ? (
                          <CheckCircle2 className={cn("w-5 h-5", isCurrent ? "text-slate-900" : "text-emerald-500")} />
                        ) : (
                          <div className="w-1.5 h-1.5 rounded-full bg-white/20" />
                        )}
                      </div>
                      <div className="text-center hidden md:block">
                        <p className={cn("text-[10px] font-black uppercase tracking-wider", isActive ? "text-white" : "text-white/30")}>{step.label}</p>
                        <p className="text-[9px] font-bold text-white/20">{step.desc}</p>
                      </div>
                   </div>
                 );
               })}
      ...
      {dispute.status === DisputeStatus.MUTUAL_RESOLUTION && (
        <MediationRoom dispute={dispute} vault={vault} role={role} onUpdate={load} />
      )}
        ...
        <main className="space-y-8">
              ...
              <CardContent className="p-8 space-y-8">
                 ...
                 <div className="grid sm:grid-cols-2 gap-4">
                    <div className="bg-slate-50 p-5 rounded-2xl border border-slate-100">
                        <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">Reason Group</p>
                        <p className="text-sm font-black text-slate-900">{getReasonLabel(dispute.reasonCode || dispute.reasonCodes?.[0] || "")}</p>
                    </div>
                    <div className="bg-slate-50 p-5 rounded-2xl border border-slate-100">
                        <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">Vault Context</p>
                        <p className="text-sm font-black text-slate-900 truncate">{vault?.title}</p>
                    </div>
                 </div>
              </CardContent>
           </Card>
           ...
           <section className="space-y-6">
              ...
              <div className="space-y-4">
                 {[...events, ...evidence].sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()).map((ev, i) => {
                   const isEvidence = !ev.type || !ev.actorRole;
                   const type = isEvidence ? "EVIDENCE" : ev.type;
                   const roleLabel = isEvidence ? role : ev.actorRole;

                   return (
                     <Card key={i} className="group border-slate-100 bg-white hover:border-emerald-200 transition-all rounded-3xl shadow-sm overflow-hidden">
                        <CardContent className="p-6 md:p-8">
                           <div className="flex flex-col md:flex-row gap-6">
                              <div className="flex md:flex-col items-center gap-3 shrink-0">
                                 <div className={cn(
                                   "w-12 h-12 rounded-2xl flex items-center justify-center border transition-all",
                                   type === "OPENED" ? "bg-amber-50 text-amber-600 border-amber-100" :
                                   type === "EVIDENCE" ? "bg-blue-50 text-blue-600 border-blue-100" :
                                   "bg-emerald-50 text-emerald-600 border-emerald-100"
                                 )}>
                                   {type === "OPENED" ? <Gavel className="w-5 h-5" /> : 
                                    type === "EVIDENCE" ? <FileSearch className="w-5 h-5" /> : 
                                 ...
                                 <header className="flex flex-wrap items-center justify-between gap-4">
                                    <div className="flex items-center gap-3">
                                       ...
                                       <span className="text-[10px] font-black text-slate-300 uppercase tracking-widest">
                                          {roleLabel} · {new Date(ev.createdAt).toLocaleDateString()}
                                       </span>
                                    </div>
                                    <p className="text-[10px] font-bold text-slate-300 font-mono">
                                       TS: {new Date(ev.createdAt).getTime().toString().slice(-6)}
                                    </p>
                                 </header>

                                 <div className="text-sm font-bold text-slate-600 leading-relaxed">
                                    {ev.payload?.note || ev.description || ev.payload?.decision || "System event logged."}
                                 </div>

                                 {/* Files/Links */}
                                 {(ev.payload?.url || ev.payload?.fileName || (ev.files || []).length > 0) && (
                                   <div className="flex flex-wrap gap-3 pt-2">
                                      {ev.payload?.url && (
                                        <a href={ev.payload.url} target="_blank" rel="noopener" className="flex items-center gap-2 px-4 py-2 bg-blue-50 text-blue-700 rounded-xl border border-blue-100 text-[11px] font-black hover:bg-blue-100 transition-all">
                                          <Link2 className="w-3.5 h-3.5" />
                                          View Shared Link
                                        </a>
                                      )}
                                      {(ev.files || []).map((file: any, k: number) => (
                                        <button key={k} onClick={() => handleDownload(file)} className="flex items-center gap-2 px-4 py-2 bg-slate-900 text-white rounded-xl text-[11px] font-black hover:bg-slate-800 transition-all shadow-lg active:scale-95">
                                          <FileText className="w-3.5 h-3.5" />
                                          {file.name}
                                        </button>
                                      ))}
                                      {ev.payload?.fileName && (
                                        <button onClick={() => handleDownload({ key: ev.payload.key, fileName: ev.payload.fileName })} className="flex items-center gap-2 px-4 py-2 bg-slate-900 text-white rounded-xl text-[11px] font-black hover:bg-slate-800 transition-all shadow-lg">
                                          <FileText className="w-3.5 h-3.5" />
                                          {ev.payload.fileName}
                                        </button>
                                      )}
           ...
           <Card className="border-none bg-white shadow-xl shadow-slate-200/50 rounded-[2.5rem] overflow-hidden sticky top-8">
              <CardHeader className="bg-slate-900 p-8">
                 <div className="flex items-center gap-3">
                    ...
                    <CardTitle className="text-white text-base font-black uppercase tracking-widest">Case Info</CardTitle>
                 </div>
              </CardHeader>
              <CardContent className="p-8 space-y-8">
                 <div className="space-y-1">
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Case Severity</p>
                    <p className="text-sm font-black text-red-500">Crucial Intervention</p>
                 </div>
                 
                 <div className="space-y-1">
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Reporting Person</p>
                    <div className="flex items-center gap-2 mt-2">
                       <div className="w-8 h-8 rounded-full bg-slate-100 flex items-center justify-center">
                          <User className="w-4 h-4 text-slate-400" />
                       </div>
                       <p className="text-sm font-black text-slate-900">{dispute.openedBy?.name}</p>
                    </div>
                 </div>

                 {dispute.requirementRef && (
                   <div className="space-y-1">
                      <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Affected Milestone</p>
                 ...
                 <div className="pt-8 border-t border-slate-100">
                    <h4 className="text-[10px] font-black text-slate-900 uppercase tracking-widest mb-4">Final Payout Rule</h4>
                    <p className="text-xs text-slate-400 font-bold leading-relaxed">
                       Once a decision is reached by the mediator or admin, funds will be programmatically split and released. This action is final and recorded on the ledger.
                    </p>
                 </div>
              </CardContent>
           </Card>
